...
export const metadata = constructMetadata({
  title: 'PL920 High-Speed Air Jet Loom | Specifications & Configuration',
  description:
    'Detailed technical specifications for PL920 High-Speed Air Jet Loom. Features 190-360cm reed widths, ELO/MTU systems, profile reed insertion, and shedding options.',
  path: '/air-jet-looms/pl920/',
});

export default function PL920Page() {
  const model = PL920_AIR_JET_LOOM;
  const breadcrumbItems = [
    { name: 'Air Jet Looms', url: '/air-jet-looms/' },
    { name: 'PL920 Air Jet Loom', url: '/air-jet-looms/pl920/' },
  ];
    ...
    <div className="space-y-12 pb-16">
      ...
      <section className="max-w-[1280px] mx-auto px-4 space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-6 space-y-6">
            <div>
              <span className="inline-block bg-blue-100 text-blue-800 text-xs font-bold px-2.5 py-1 rounded uppercase tracking-wider">
                Air Jet Weaving System
              </span>
      ...
      <section id="specifications" className="max-w-[1280px] mx-auto px-4 space-y-6">
        <div className="space-y-2">
          ...
          <h2 className="text-2xl font-bold text-slate-900">PL920 Technical Specifications</h2>
        </div>
      ...
      <section className="max-w-[1280px] mx-auto px-4 space-y-6">
        <div className="space-y-2">
          ...
          <h2 className="text-2xl font-bold text-slate-900">PL920 Major Operating Subsystems</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {model.majorSystems.map((sys, idx) => (
            <div key={idx} className="bg-white border border-slate-200 rounded-lg p-6 space-y-2 shadow-sm">
              <h3 className="font-bold text-slate-900 text-base text-blue-800">{sys.name}</h3>
      ...
      <section className="max-w-[1280px] mx-auto px-4 space-y-6">
        <div className="border-t border-slate-200 pt-8 flex justify-between items-center">
          ...
          <Link href="/water-jet-looms/" className="text-xs font-semibold text-blue-700 hover:text-blue-900 flex items-center gap-1">
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div>
            <span className="text-xs font-bold text-cyan-800 uppercase">Synthetic Filament Weaving</span>
            <h3 className="text-lg font-bold text-slate-900">{PL5810_WATER_JET_LOOM.name}</h3>
            <p className="text-xs text-slate-600 mt-1 max-w-xl">
              For high-speed synthetic filament weaving up to 1100 RPM design max speed.
            </p>
          </div>
          <Link
